...
function toggleVisibilityCheckbox(type) {
  if (type === 'student') {
    ...
    document.getElementById('evVisibilityStudent').checked = false;
    document.getElementById('evVisibilityAll').checked = true;
  }
}
...
async function saveEvent() {
  ...
  const visibility = document.getElementById('evVisibilityAll').checked ? 'all' : 'student';
  const body = {
...
async function submitImport() {
  ...
  try {
    ...
    // Build toast
    let toastMsg = '';
    if (data.imported > 0)  toastMsg += `✅ ${data.imported} new student${data.imported !== 1 ? 's' : ''} added. `;
    ...
    if (!toastMsg) toastMsg = 'ℹ️ No new students added.';
