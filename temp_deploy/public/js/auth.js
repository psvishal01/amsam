...
if (document.getElementById('loginForm')) {
  ...
  if (getToken()) {
    const u = getUser();
    let dest = '/home.html';
    if (isAdmin()) dest = '/admin.html';
    else if (u?.role === 'guest') dest = '/guest-dashboard.html';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      let dest = '/home.html';
      if (['super_admin','sub_admin'].includes(data.user.role)) dest = '/admin.html';
      else if (data.user.role === 'guest') dest = '/guest-dashboard.html';
      window.location.href = dest;
    } catch (err) {
